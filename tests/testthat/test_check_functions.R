...
test_that("Exceptions are thrown", {
  ...
  expect_error(.checkArgs(model, metric = "aicc", hypers = h),
  ...
  expect_error(.checkArgs(model_cv, metric = "aicc", hypers = h),
  ...
  h$a <- NULL
  expect_error(.checkArgs(model, metric = "auc", test = val, hypers = h), NA)
  # Throws exception if provided hypers are not tunable
  h <- list("fc" = c("l", "lq", "lqp"), "lambda" = c(500, 600))
  expect_error(.checkArgs(model, "auc", val, hypers = h),
