...
test_that("The function .check_args function raises exceptions", {
  ...
  expect_snapshot(.check_args(model, metric = "aicc", hypers = h), error = TRUE)
  ...
  # is not provided
  expect_snapshot(.check_args(model, metric = "auc", hypers = h), error = TRUE)
  # Throws exception if provided hypers are not tunable
  h <- list("fc" = c("l", "lq", "lqp"), "lambda" = c(500, 600))
  expect_snapshot(.check_args(model, "auc", data, hypers = h), error = TRUE)
