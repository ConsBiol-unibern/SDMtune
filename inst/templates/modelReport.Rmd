...
message("save files...")
swd2csv(params$model@presence, "presence.csv")
swd2csv(params$model@background, "background.csv")
if (!is.null(test))
  swd2csv(test, "test.csv")
```
...
suppressMessages(ggsave(filename = "ROC_curve.png", plot = plot, device = "png", path = params$folder))
...
if (params$response_curves) {
  ...
  for (var in vars) {
    ...
    suppressMessages(ggsave(filename = paste0(var, "_marginal.png"), plot = plot, device = "png", path = params$folder))
    path <- paste0(params$folder, "/", var, "_marginal.png")
    element <- paste0("<a href=\"", path, "\"><img src=\"", path, "\" width=25%></a>")
...
if (params$response_curves) {
  ...
  for (var in vars) {
    ...
    suppressMessages(ggsave(filename = paste0(var, ".png"), plot = plot, device = "png", path = params$folder))
    path <- paste0(params$folder, "/", var, ".png")
    element <- paste0("<a href=\"", path, "\"><img src=\"", path, "\" width=25%></a>")
    elements <- c(elements, element)
    pb$tick(1)
  }
...
if (jk) {
  ...
  suppressMessages(ggsave(filename = "train_jk.png", plot = plot, device = "png", path = params$folder))
  if (!is.null(test)) {
    plot <- plotJk(jk, type = "test", ref = auc(params$model, test))
    suppressMessages(ggsave(filename = "test_jk.png", plot = plot, device = "png", path = params$folder))
