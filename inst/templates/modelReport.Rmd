...
if (params$response_curves) {
  ...
  elements <- c()
  for (var in vars) {
    plot <- plotResponse(params$model, var, type = params$type, marginal = TRUE, rug = TRUE, clamp = params$clamp)
...
if (params$response_curves) {
  ...
  elements <- c("")
  for (var in vars) {
    plot <- suppressMessages(plotResponse(params$model, var, type = params$type, marginal = FALSE, rug = TRUE, clamp = params$clamp))
...
suppressMessages(ggsave(filename = "map.png", plot = plot, device = "png", path = plot_folder))
