...
message(crayon::green(cli::symbol$tick), " File saved")
```
...
message(crayon::green(cli::symbol$tick), " ROC curve")
```
...
message(crayon::green(cli::symbol$tick), " Thresholds")
```
...
message(crayon::green(cli::symbol$tick), " Marginal response curves")
```
...
message(crayon::green(cli::symbol$tick), " Univariate response curves")
```
`r if(!params$response_curves) {"-->"}`
...
message(crayon::green(cli::symbol$tick), " Variable importance")
```
...
message(crayon::green(cli::symbol$tick), " Jackknife test")
```
`r if(!params$jk) {"-->"}`
...
* Regularization multiplier: `r params$model@model@reg``r if(class(params$model@model) == "Maxent") {paste("\n* Extra arguments:", paste(params$model@model@extra_args, collapse = ", "))}`
