...
.plot_report_roc(params, plot_folder)
```
...
.plot_report_response_curves(params, type = "marginal", plot_folder = plot_folder)
```

### Univariate curves:

```{r univariate response, message=FALSE, echo=FALSE, warning=FALSE, eval=params$response_curves}
.plot_report_response_curves(params, type = "univariate", plot_folder = plot_folder)
```
...
.make_report_prediction(params, plot_folder)
```
...
.compute_report_jk(params, test, plot_folder)
```
