...
presence <- data.frame(x = data$lon, y = data$lat)
presence <- presence[!duplicated(presence), ]  # Remove duplicate rows
presence <- presence[!is.na(presence$x) | !is.na(presence$y), ]  # Remove NA values
```

```{r}
cat('Number of observations before cleaning data:', nrow(data),
    '\nNumber of obsevations after cleaning data  :', nrow(presence))
```


We downloaded `r nrow(data)` presence locations and after cleaning the duplicates and the NA we have `r nrow(presence)` locations for our analysis. We can plot the study area and the presence locations we downloaded from GBIF:

```{r, fig.align="center"}
ggplot(map_data('world'), aes(long, lat)) +
    geom_polygon(aes(group = group), fill = "grey95", color = "gray40", size = 0.2) +
    geom_jitter(data = presence, aes(x = x, y = y), color = 'red',
                alpha = 0.4, size = 2) +
...
c(train_coords, val_coords, test_coords) %<-% trainValTest(presence, val = 0.2, test = 0.2, seed = 25)
cat('Total presence locations  :', nrow(presence),
...
Let's have a look at the `MaxentSWD` object:

```{r show MaxentSWD object}
train
```
...
You can also save the `MaxentSWD` object in a **.csv** file to use t directly in the software **Maxent**.
...
```{r run MaxEnt}
full_model <- runMaxent(train, bg_model, rm = 1, fc = 'lqph', test = test, output_format = 'cloglog', response_curves = TRUE, folder = 'full_model')
```
...
The result of the `runMaxent` function is an object of class `MaxentModel` that stores in different slots the value of `presence`, `background`, MaxEnt `results`, `lambdas` coefficients, `rm`, `fc`, `iterations`, the `path` where is stored the MaxEnt output, the path of the `html` file and the `plot_data` if you set the `response_curves` parameter to `TRUE` (some of this values come from the results of the [maxent](https://www.rdocumentation.org/packages/dismo/versions/1.1-4/topics/maxent} function of `dismo`)). You can access this values using `model@name_of_the_slot`, like `model@rm` or `model@fc`. When you call the model object R prints all the parameters you used to run the model and open the **html** file in your browser if you save the results permanetly using the parameter `folder =`.
<div style="text-align: right"><a href="#top">Back to top</a></div>

# Make Prediction

When you use `runMaxent` the function trains a model but doesn't make any prediction. To make prediction use `predictMaxent`:
