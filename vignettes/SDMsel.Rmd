...
The model with the highest validation AUC uses a `regularization multiplier` equal to 0.1 but at rm = 0.3 we have the minimum differece between train and validation AUC, we select this model from the models slot and use it to tune the number of iterations.

```{r}
tuned_rm <- tune_rm@models[[which.min(tune_rm@results$diff_AUC)]]
```
<div style="text-align: right"><a href="#top">Back to top</a></div>
...
```{r, message=F}
tune_it <- tuneIt(tuned_rm, its = seq(400, 2300, 100), metric = "auc")
```
...
The test AUC changes varying the number of iterations and reach the maximum using 2200 iterations, we select this model from the models slot:
```{r}
