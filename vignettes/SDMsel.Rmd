...
We use a sub sample of the background locations to train the model and the full dataset to compute the correlation between the environmental variables. We extract the sub sample using the `getSubsample` function and after we plot the retained background locations:
```{r, fig.align="center"}
...
The best number of background locations is 3000, we select this model from the models slot and use it to tune the regularization multiplier.
```{r}
...
The default number of iterations used by the optimization algorithm in MaxEnt is 500. This value is in general good but can be tuned like other hyperparameters. We test a sequence iterations starting from 300 till 600 with increments of 25:
```{r, message=F}
...
The test AUC changes varying the number of iterations and reach the maximum using 400 iterations, we select this model from the models slot:
```{r}
