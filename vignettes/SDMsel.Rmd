...
```{r run MaxEnt}
base_model <- trainMaxent(presence = train, bg = bg_model, rm = 1, fc = "lqph", type = "cloglog", test = val, folder = "base_model")
```
...
```{r jk, message=F}
jk <- doJk(base_model, with_only = T, metric = "auc")
```
...
```{r make prediction}
prediction <- predict(base_model, data = predictors, filename = "base_model_prediction", format = "GTiff", overwrite = T)
```
...
selected_var_model <- varSel(base_model, bg, metric = "auc", rm = 0.001, method = "spearman", cor_th = 0.75, use_permutation = T)
```
...
`SDMsel` provides a family of functions to tune model hyperparameters. For `Maxent` models we can tune the **feature combination**, the number of **background locations**, the **regularization multiplier** and the **number of iterations**. All the function of this family return an object of class `SDMsel` with two slots, one for the results and one with all the trained models.
...
plot_ly(tune_fc@results) %>%
  add_trace(x = ~fc, y = ~train_AUC, type = "bar", name = "Train AUC", marker = list(color = "#00BFC4")) %>%
  add_trace(x = ~fc, y = ~test_AUC, type = "bar", name = "Validation AUC", marker = list(color = "#F8766D")) %>%
  layout(xaxis = list(title = "Feature Class Combination"), yaxis = list(title = "AUC"), barmode = "group", title = "Model performance")
```
```{r, fig.align="center", echo=F}
p <- plot_ly(tune_fc@results) %>%
  add_trace(x = ~fc, y = ~train_AUC, type = "bar", name = "Train AUC", marker = list(color = "#00BFC4")) %>%
  add_trace(x = ~fc, y = ~test_AUC, type = "bar", name = "Validation AUC", marker = list(color = "#F8766D")) %>%
  layout(xaxis = list(title = "Feature Class Combination"), yaxis = list(title = "AUC"), barmode = "group", title = "Model performance")
...
```
```{r}
tune_fc@results[which.max(tune_fc@results$test_AUC), ]
```

The Feature Class combination with the highest validation AUC is **lqpht**, we select this model from the models slot and use it to tune the number of background locations.
```{r}
tuned_fc <- tune_fc@models[[which.max(tune_fc@results$test_AUC)]]
```
<div style="text-align: right"><a href="#top">Back to top</a></div>

## Tune number of background locations

The model we ran before used the 5000 background locations. To test the effect of using different number of background locations on the model performance we can run the function `tuneBg`. We have a maximum of `r nrow(bg@data)` locations in the `bg` object, in the next example we train a sequence of models using `r c(seq(3000, 9000, 1000), nrow(bg@data))`
```{r, message=F}
...
plot_ly(tune_bg@results) %>%
  ...
  layout(xaxis = list(title = "Background locations"), yaxis = list(title = "AUC"), title = "Model performance")
```
```{r, fig.align="center", echo=F}
p <- plot_ly(tune_bg@results) %>%
  add_trace(x = ~bg, y = ~train_AUC, type = "scatter", mode = "lines", name = "Train AUC", marker = list(color = "#00BFC4")) %>%
  add_trace(x = ~bg, y = ~test_AUC, type = "scatter", mode = "lines", name = "Validation AUC", marker = list(color = "#F8766D")) %>%
  layout(xaxis = list(title = "Background locations"), yaxis = list(title = "AUC"), title = "Model performance")
htmltools::div(p, align = "center")
```

The best number of background locations is 2000, we select this model from the models slot and use it to tune the regularization multiplier.
```{r}
tuned_bg <- tune_bg@models[[which.max(tune_bg@results$test_AUC)]]
```
<div style="text-align: right"><a href="#top">Back to top</a></div>

## Tune Regularization Multiplier

In the next step we tune the `regularization multiplier`. The default regularization multiplier used by MaxEnt is 1, but is not always the best choice:
...
```{r show results tuneRM, eval=F}
tune_rm@results
```
```{r}
kable(tune_rm@results) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "responsive")) %>%
...
plot_ly(tune_rm@results) %>%
  ...
  layout(xaxis = list(title = "Regularization multiplier"), yaxis = list(title = "AUC"), title = "Model performance")
```
```{r, fig.align="center", echo=F}
p <- plot_ly(tune_rm@results) %>%
  add_trace(x = ~rm, y = ~train_AUC, type = "scatter", mode = "lines+markers", name = "Train AUC", marker = list(color = "#00BFC4")) %>%
  add_trace(x = ~rm, y = ~test_AUC, type = "scatter", mode = "lines+markers", name = "Validation AUC", marker = list(color = "#F8766D")) %>%
  layout(xaxis = list(title = "Regularization multiplier"), yaxis = list(title = "AUC"), title = "Model performance")
htmltools::div(p, align = "center")
```
The highest test AUC uses a `regularization multiplier` equal to 1.1, we select this model from the models slot and use it to tune the number of iterations.
```{r}
tuned_rm <- tune_rm@models[[which.max(tune_rm@results$test_AUC)]]
```
<div style="text-align: right"><a href="#top">Back to top</a></div>

## Tune number of iterations:

The default number of iterations used by the optimization algorithm in MaxEnt is 500. This value is good for not complex models and can be tuned as other hyperparameters:
```{r, fig.align="center"}
tune_it <- tuneIt(tuned_rm, its = seq(300, 600, 25), metric = "auc")
```
...
plot_ly(tune_it@results) %>%
  ...
  layout(xaxis = list(title = "Number of iterations"), yaxis = list(title = "AUC"), title = "Model performance")
```
```{r, fig.align="center", echo=F}
p <- plot_ly(tune_it@results) %>%
  add_trace(x = ~it, y = ~train_AUC, type = "scatter", mode = "lines+markers", name = "Train AUC", marker = list(color = "#00BFC4")) %>%
  add_trace(x = ~it, y = ~test_AUC, type = "scatter", mode = "lines+markers", name = "Validation AUC", marker = list(color = "#F8766D")) %>%
  layout(xaxis = list(title = "Number of iterations"), yaxis = list(title = "AUC"), title = "Model performance")
htmltools::div(p, align = "center")
...
The test AUC increases changing the number of iterations from the default value of 500, and reach the maximum using 400 iterations, we select this model from the models slot:
```{r}
tuned_model <- tune_it@models[[which.max(tune_it@results$test_AUC)]]
```
<div style="text-align: right"><a href="#top">Back to top</a></div>
...
tuned_model
```
All the functions of the tuning family don't save the MaxEnt ouput but we could save it using the `trainMaxent` function and setting all the hyperparameter values of the previous output. The `maxentOutput` function make this step easier, we just have to pass the model and it will set the hyperparameters for us. In addition there are other parameters to add the response curves and the JAckknife test to the html output:
```{r}
maxentOutput(tuned_model, response_curves = T, jk = T, threads = 4, folder = "final_output")
```
