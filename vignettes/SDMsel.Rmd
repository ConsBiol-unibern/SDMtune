...
Before loading `SDMsel` you have to increase the RAM that **MaxEnt** can use. You should leave 2G for your OS and the rest you can use for Maxent. If you have 4GB RAM use `-Xmx2g` in the following command: 
...
select_var_model <- varSel(full_model, bg, rm = 0.001, method = "spearman", cor_th = 0.75, use_permutation = TRUE)
```
```{r, echo=F}
select_var_model <- suppressMessages(varSel(full_model, bg, rm = 0.001, method = "spearman", cor_th = 0.75, use_permutation = TRUE))
```
