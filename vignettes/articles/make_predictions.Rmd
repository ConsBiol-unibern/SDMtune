...
In the previous articles you have learned how to [prepare the data](./prepare_data.html) for the analysis and how to [train a model](./train_model.html) using **SDMtune**. In this article you will learn how to use a trained model to make predictions. We will use the `default model` that we trained in the previous article to predict the distribution of the Andean condor.
...
Next we get the prediction for our `presence` locations using the **cloglog** output type:
...
The `plotPred()` function plots a map with a color ramp similar to the one used by the MaxEnt Java software. We can pass additional arguments to customize the map. In the next example we provide a custom color ramp and we add a title to the legend:
...
In the [next article](./evaluate_model.html) you will learn how to evaluate a model.
