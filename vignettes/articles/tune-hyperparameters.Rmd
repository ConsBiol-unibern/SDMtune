...
Let's say we want to use the best tuned model found by the `randomSearch()` function. Before evaluating the model using the testing dataset, we can merge the training and the validation datasets together to increase the number of locations and train a new model with the merged observations and the tuned configuration. At this point we may have removed variables using the `varSel()` or `reduceVar()` function, so we cannot merge directly the initial datasets, as they contain all the environmental vavriables. We can extract the train dataset with the selected variables from the output of the experiment and merge it using the `mergeSWD()` function:
```{r merge}
...
merged_data <- mergeSWD(new_train, val)
```

The `val` dataset contains all the initial environmental variables but the `mergeSWD()` function will merge only those that are present in both datasets (i.e. the ones that are present in the `new_train` dataset, see warning message). 
...
The best model is at row `r index` and was trained using **`r exp_6@results[index, 1]`** feature class combination, **`r exp_6@results[index, 2]`** as regularization multiplier and **`r exp_6@results[index, 3]`** number of iteration:
```{r final model}
final_model <- train("Maxnet", data = merged_data, fc = exp_6@results[index, 1], reg = exp_6@results[index, 2], iter = exp_6@results[index, 3])
```
