...
The `train()` function accepts folds created with other packages (ENMeval and blockCV). The function will convert internally into the correct format for `SDMtune`. These packages have specific function to create folds partitions that are spatially or environmentally indipendent.
