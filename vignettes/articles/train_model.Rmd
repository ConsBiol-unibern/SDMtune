...
In the [previous article](./prepare_data.html) you have learnt how to prepare the data for the analysis using the [condor](../../reference/condor.html) dataset and the [WorldClim](http://www.worldclim.org/) environmental variables. Now it's time to train your first model, let's do it!  
**SDMtune** at the moment supports only the Maximum Entropy method with two differnt implementations:

* **Maxent** using the `dismo` package [@Hijmans2017];
* **Maxnet** using the `maxnet` package [@Phillips2017].
...
* linear, quadratic, product and hinge feature combinations;
...
The function `train()` accepts optional arguments that can be used to change the default model settings. In our previous example we could also train the model using:
...
### Try yourself
Try to change the default settings  and train a model using linear and hinge as feature combinations, 0.5 as regularization multiplier and 700 iterations. To see the solution hightlight the next cell:
...
In the [next article](./make_predictions.html) you will learn how to make prediction.
