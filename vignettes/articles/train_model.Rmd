...
* `train`: the `SWD()` object with the presence and background locations we created in the previous article.
...
default_model <- train(method = "Maxent", data = train, fc = "lqph", reg = 1, iter = 500)
```
...
model <- train(method = "Maxent", data = train, fc = "lh", reg = 0.5, iter = 700)
```
