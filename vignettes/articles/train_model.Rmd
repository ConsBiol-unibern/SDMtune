...
In the [previous article](./prepare_data.html) you have learned how to prepare the data for the analysis using the [virtualSp](../../reference/virtualSp.html) dataset and the [WorldClim](http://www.worldclim.org/) environmental variables. Now it's time to train your first model, let's do it!  
**SDMtune** supports the following methods: Artificail Neural Networks, Boosted Regression Trees, Maximum Entropy and Random Forest. The following are the packages used for the different methods:
...
The code necessary to train a model is the same for all the implementations. We will show how to train a **Maxent** model, you can adapt the code for the other methods.
