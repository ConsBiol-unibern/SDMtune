...
```
```{r, echo=F}
base_model <- trainMaxent(presence = train, bg = bg_model, rm = 1, fc = "lqph", type = "cloglog", test = val, iter = 500)
```
...
```
```{r, echo=F}
prediction <- predict(base_model, data = predictors)
```
