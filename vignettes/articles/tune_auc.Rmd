...
To create a presence absence map we have to choose a threshold. The function `thresholds` provides a list of possible thresholds:
...
selected_var_model <- varSel(base_model, bg4cor =  bg, metric = "auc", rm = 0.001, method = "spearman", cor_th = 0.75, permut = 2)
```

Note that we used **permut = 2** only to reduce the compuation time but you can increment it to have a better estimate of the permutation importance. The function returns the model trained without the correlated variables, if we print the object we see that the function removed **bio6**, **bio7**, **bio8** and **bio16**:
...
`SDMsel` provides a family of functions to tune model hyperparameters: **feature combination**, number of **background locations** and **regularization multiplier**. All the functions of this family return an object of class `SDMtune` with two slots, one for the results and the other with all the trained models.
...
```{r tune rm, message=F}
tune_rm <- tuneRM(tuned_bg, rms = seq(0.1, 2, 0.1), metric = "auc", test = val)
```
...
The model with the highest validation AUC uses a `regularization multiplier` equal to 0.5 but at rm = 0.9 we have a lower differece between train and validation AUC with an irrelevant decrease of validation AUC, we select this model from the models slot.
...
We decide we want to remove the variables with a permutation importance lower than 5\% but we do it only if the model performance doesn't decrease and we use the **AUC** of the Validation dataset to measure the model performance:
...
but **bio17** and **bio12** were not removed because the model without **bio17** has lower performance than the one with. We can demonstrate it removing the variable and comparing the **AUC** values on the Validation dataset:
...
All the previous functions don't save the model ouput but we could save it using `modelReport`. The function creates a report similar to the one generated by MaxEnt software and gives the possibility to enable or disable the response curves and the Jackknife test:
