...
```{r threshold}
thresholds(base_model, type = "cloglog")
```
...
All the previous functions don't save the model ouput but we could save it using the `maxentOutput`. The function makes this step easy, we just have to pass the model and it will set the hyperparameters for us. In addition there are other parameters to add the response curves and the Jackknife test to the html output:

```{r, eval=F}
modelOutput(tuned_model, type = "cloglog", test = val, response_curves = T, jk = T, threads = 4, folder = "final_output")
```
