...
For this example will use a **Maxnet** model and a training/testing validation strategy like in the [previous article](./evaluation_strategies.html#train-and-testing-datasets):
```{r train test}
...
In the case of an `SDMmodelCV()` the response curve shows the averaged value of the prediction together with the one Standard Deviation error interval. We use the cross validation model trained in the [previous article](./evaluation_strategies.html#cross-validation)
...
All what you have learned till now con be saved and summarized calling the function `modelReport()`. The function will create a folder with the provided name and:
