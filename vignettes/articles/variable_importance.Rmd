...
```{r train maxnet, eval=FALSE}
maxnet_model <- train("Maxnet", p = presence, a = bg_model)
```
```{r train maxnet bis, echo=FALSE}
maxnet_model <- SDMtune:::bm_maxnet
```
...
Another method to estimate the variable importance is the leave one out Jackknife test. Here we remove one variable at time and we record the change in the chosen metric. 
