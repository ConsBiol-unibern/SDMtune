...
All the previous artivcles are based on presence only methods, in this article you will learn how to train a presence absence model. The following examples are based on the Artificial Neural Networds method [@Venables2002], but you can adapt the code for any of the other supported methods.
...
We first train the model with default settings and running the cross validation:
```{r}
set.seed(25)
...
To check which hyperparameters can be tuned we use the function `get_tunable_args()` function:
```{r get tunable args}
get_tunable_args(model)
```

We use the `optimizeModel()` function to find the best hyperparameter combinations for the learning rate, the number of trees and the bag fraction:

```{r optimize model}
h <- list(size = 2:20, decay = c(0.01, 0.05, 0.1, 0.3, 0.5), maxit = c(100, 300, 500))
...
We now train a model with the same configuration as found by the `optimizeModel()` function without cross validation, using all the train data, and we evaluate it using the held apart testing dataset:
