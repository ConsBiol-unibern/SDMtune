...
In the previous articles you have learnt how to [prepare the data](./prepare_data.html) for the analysis, how to [train a model](./train_model.html) and how to [make predictions](./make_predictions.html) using **SDMtune**. In this article you will learn how to evaluate your model using thre different metrics. We will compute the value of the metrics on the training dataset using the `default model` that we trained in a previous article and explain how to evaluate the model in the testing dataset later in another article.
...
For the Akaike Information Criterion corrected for small sample size we use the function `aicc()`. In this case we need to pass the 'predictors' raster `stack` object we created in the [first article](./prepare_data.html#acquire-environmental-variables) to the `env` argument:
```{r aicc}
